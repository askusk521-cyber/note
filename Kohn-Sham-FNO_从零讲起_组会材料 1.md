...
- 一个空间场 $v(\mathbf{r})$，做傅里叶变换后变成频率分量 $\hat{v}(\mathbf{k})$（$\mathbf{k}$ 是倒空间/reciprocal space 里的波矢）：

$$
\hat{v}(\mathbf{k}) = \int v(\mathbf{r})\,e^{-i\mathbf{k}\cdot\mathbf{r}}\,d\mathbf{r}
\qquad\Longleftrightarrow\qquad
v(\mathbf{r}) = \int \hat{v}(\mathbf{k})\,e^{i\mathbf{k}\cdot\mathbf{r}}\,d\mathbf{k}
$$

- $|\mathbf{k}|$ 越小 = 波长越长 = **低频 = 全局、长程结构**；$|\mathbf{k}|$ 越大 = **高频 = 局部、细节**。
- 量子化学里 $v_{\text{KS}}$ 建在**均匀实空间网格**上，Hartree 势 $v_{\text{H}}$ 本质是一个 **$1/r$ 卷积**（长程相互作用）：

$$
v_{\text{H}}(\mathbf{r}) = \int \frac{n(\mathbf{r}')}{|\mathbf{r}-\mathbf{r}'|}\,d\mathbf{r}'
\;=\;\big(g * n\big)(\mathbf{r}),\qquad g(\mathbf{r})=\tfrac{1}{r}
$$

- **卷积定理（convolution theorem）**：实空间里的卷积 = 频域里的逐点乘法：

$$
\widehat{(g * n)}(\mathbf{k}) \;=\; \hat{g}(\mathbf{k})\cdot \hat{n}(\mathbf{k})
$$

  所以长程、非局部的相互作用，在频域里变得极其便宜（乘一下就完事）。
